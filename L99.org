...
- Assume there is only one valid solution for each puzzle.
...
Once the information about sites got organized, we will get a simple constraint satisfaction problem (CSP) to solve. The target is to assign proper values to the sites to satisfy all cross points have no conflicts. CSP with dependency cycles is NP-hard because it is hard to handle circular dependencies. For example, in p99a, there is no circular dependencies, so you can even solve it with a tree DFS on sites (but that won't work for p99b). Actually it is very hard to do pruning on sites in this problem, because assigned sites are not identical to tree nodes.

Note that crossword is stronger than general CSP, because it is restricted to a planar grid embedding problem. Here we use a fixed-point propagation algorithm, starting from a point, recursively compute possible options for its constraint sites, finally decide its possible options from its constraints. I have an hypothesis that if there is only one unique solution, this algorithm should be able to converge in one pass. (not proven yet)

At least, we can say that this algorithm doesn't ensure convergency if there are no fixed points found in one pass. For example, consider there are two parallel solutions that have no conflicts to each other, both can be applied, but differs at cross points, then we won't be able to filter out a concrete solution after propagation. To find out a solution from non-convergent cases, we may do one more propagation with one more fixed point picked, and recurse until the whole graph converge to one single solution. I didn't implement this part, because one pass already worked for the 4 examples given, and I didn't find more examples to test.

Also note that characters at non-cross points are free, words with the same characters at cross points are identical. If the dictionary is large, we may shrink each site's dictionary to the equivalent classes of constraints. This may help accelerate convergency of fixed-point propagation.
...
#+begin_src lisp :results output verbatim :wrap src plaintext
  ...
  (defun crossword-dfs (wordmap sitemap path d x y)
    (let* ((site (aref sitemap d x y)))
      (setf (aref wordmap d x y)
            (filter-words
             (aref wordmap d x y)
             (loop for a in (cdr site)
      ...
      (multiple-value-bind (dict dotmap)
        ...
        (multiple-value-bind (sitemap sites)
          ...
          (let* ((ltb (build-length-table dict))
            ...
            (let ((site (car sites)))
              (when (crossword-dfs wordmap sitemap nil
