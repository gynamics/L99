...
Note that, although this function is very simple, it is not tail-recursive, the accumulation must be done after ~my-length~ returns 0 in recursion. Maybe a smarter compiler can translate it into iteration form, if it can identify that ~1+~ is a monotonic, pure, endomorphism. But this is beyond simple TCO (tail call optimization). If you don't want to add extra type declarations, you may prefer write it with ~reduce~.

#+begin_src lisp
  (defun my-length (l)
    (reduce (lambda (acc _) (1+ acc)) l :initial-value 0))
#+end_src

There are many similar cases in solutions for following problems, I just add a note here once.
...
P. S. Note that CBAL tree is not a complete tree. A completed tree is a tree filled level by level, from top to bottom, from left to right. The number of CBAL trees with given number of nodes satisfy ~a(2n) = 2*a(n) * a(n-1), a(2n+1) = a(n)*a(n)~. See [[https://oeis.org/A110316]] for more information.
...
Note that a symmetric binary tree must have odd number nodes, so there is no solution for even numbers.

#+begin_src lisp
  (defun sym-cbal-trees-count (n)
    (length
     (mapcan (lambda (tree) (when (symmetric tree) (list tree)))
             (cbal-tree n))))

  (sym-cbal-trees-count 57)
#+end_src

 #+RESULTS:
- 256

Note that, suppose the number of symmetric cbal trees with n nodes is s(n), the number of cbal trees with n nodes is a(n), note that, ~s(2*n+1) = a(n)~.
